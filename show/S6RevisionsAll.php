...
try{
	...
	$data = $pdo->query("SELECT * FROM S6Revision ORDER BY g,flag,idReviser,dateT")->fetchAll();	//"SELECT * INTO OUTFILE 'mydata.csv' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '".'"'."' LINES TERMINATED BY '"."\n"."' FROM mtkCrwRevision GROUP BY idGroup,flag,idReviser,dateT")->fetchAll();//
	...
	foreach ($data as $row) {
		...
		echo ("<tr class='cRow'><td class='commentWindow small'>"
            ...
            . $row["idRevision"]."</td><td class='commentWindow'>"
